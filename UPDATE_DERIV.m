...
% function [X,dX] = UPDATE_DERIV(Moment, Thrust, state)
% global m Ixx Iyy Izz
% % state = [pos vel ang ang_vel]
% % dstate = [dpos dvel dang dang_vel]
% 
...
% 
% % dpos
% % dpos = Rotate_vec(vel', ang, 'b2n');
% C_b2n = [1 0 0; 0 cosd(ang(1)) sind(ang(1)); 0 -sind(ang(1)) cosd(ang(1))]...
%     *[cosd(ang(2)) 0 sind(ang(2)); 0 1 0; sind(ang(2)) 0 cosd(ang(2))]...
%     *[cosd(ang(2)) sind(ang(3)) 0; -sind(ang(3)) cosd(ang(3)) 0; 0 0 1];   % 3()-2-1
% 
% dpos = C_b2n*vel';
% % dvel
% P = ang_vel(1);
...
% W = vel(3);
% 
% % Thrust = C_n2b*acc';  % b-frame
% g_n = [0 0 1.63]';
% g = Rotate_vec(g_n, ang, 'n2b');
% 
% dvel = -[Q*W-R*V;R*U-R*W;P*V-Q*U] + Thrust'/m + g;
% 
% % dang
% dang = [1 sin(ang(1))*tan(ang(2)) cos(ang(1))*tan(ang(2));...
%     0 cos(ang(1)) -sin(ang(1));...
...
% % dang_vel
% I = [Ixx 0 0; 0 Iyy 0; 0 0 Izz];
% %dang_vel = inv(I)*[Moment(1)/Ixx + (Iyy-Izz)*Q*R; Moment(2)/Iyy + (Izz-Ixx)*R*P;...
% %    Moment(3)/Izz + (Ixx-Iyy)*P*Q];
% dang_vel = inv(I)*[Moment(1) + (Iyy-Izz)*Q*R; Moment(2) + (Izz-Ixx)*R*P;...
%     Moment(3) + (Ixx-Iyy)*P*Q];
% 
% X = state;
